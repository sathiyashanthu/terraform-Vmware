...
data "vsphere_virtual_machine" "clone_machine" {
  name          = var.clone_machine_name
  datacenter_id = data.vsphere_datacenter.dc.id
}
...
resource "vsphere_virtual_machine" "ubu-testing" {
  ...
  guest_id         = data.vsphere_virtual_machine.clone_machine.guest_id
  ...
  disk {
    label            = data.vsphere_virtual_machine.clone_machine.disks[0].label
    thin_provisioned = data.vsphere_virtual_machine.clone_machine.disks[0].thin_provisioned
    size             = var.disk_size
  }


  clone {
    template_uuid = data.vsphere_virtual_machine.clone_machine.id
      }
}